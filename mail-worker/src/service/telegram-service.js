...
const telegramService = {
	...
	async handleWebhook(c, body) {
		...
		const sendTelegramReplyRaw = async (text, replyMarkup = null) => {
		...
		try {
			...
			await sendTelegramReplyRaw(`❌ <b>系统运行异常:</b> ${globalErr.message}\n<pre>${globalErr.stack}</pre>`);
		}
	}
